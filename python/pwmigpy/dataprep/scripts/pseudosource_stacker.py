...
def load_special_attributes(
    ensemble, algorithm, argdoc, clusterdoc
) -> SeismogramEnsemble:
    """
...
def set_weights(
    dataset, wt_control, summary_weight_output_key="weight", magnitude_weight_key=None
) -> dict:
    """
    ...
    for dkey in dataset.keys():
        ...
        ens = bsm.set_ensemble_summary_weights(
            ens, summary_weight_key_list, summary_weight_output_key
        )
        dataset[dkey] = ens
    ...
    for d in stacked_data.member:
        if d.live:
            ...
            arrivals = refmodel.get_travel_times(
                source_depth_in_km=srcdepth, distance_in_degree=dist, phase_list=["P"]
            )
            ...
            if len(arrivals) >= 1:
                ...
                message = (
                    "Failed to compute P wave arrival time to set timing for this stack"
                )
                d.kill()
...
def special_index_state(
    db, index_name="pseudosource_stacker_index", keys=["data_tag", "source_id"]
) -> int:
    """
...
def verify_wfindex(db, verbose=False):
    ...
    match index_state:
        ...
        case 1:
            if verbose:
                print(
                    "index with name pseudosource_stacker_index exists in correct form - no action needed"
                )
        case 0:
            if verbose:
                print("pseudosource_stacker_index does not exist - creating it now")
                db.wf_Seismogram.create_index(
                    [("data_tag", 1), ("source_id", 1)],
                    name="pseudosource_stacker_index",
                )
        case _:
        ...
        if control.enabled(algorithm):
            ...
            match algorithm:
                case "average":
                    ...
                    stacked_data = bsm.stack_groups(
                        dataset, method=algorithm, janitor=janitor
                    )
            ...
            if stacked_data.live:
                ...
                stacked_data = load_special_attributes(
                    stacked_data,
                    algorithm,
                    argdoc,
                    clusterdoc,
                ...
                stacked_data = normalize(
                    stacked_data, site_matcher, handles_ensembles=False
                )
...
def main(args=None):
    ...
    site_matcher = ObjectIdMatcher(
        db, collection="site", attributes_to_load=["_id", "lat", "lon", "elev"]
    )
    pffile = args.pffile
    ...
    if N == 0:
        ...
        raise RuntimeError(message)
    print(
        "pseudosource_stacker:  working on {} source clusters defined in telecluster collection".format(
            N
        )
    )
    if verbose:
