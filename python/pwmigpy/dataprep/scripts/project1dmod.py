...
from pwmigpy.db.database import GCLdbsave, GCLdbread, vmod1d_dbread
...
def main(args=None):
    ...
    if n != 1:
        if n > 1:
            print(
                "project1dmod(WARNING):  duplicate documents for for query=",
                query,
            ...
            print(
                "project1dmod(Fatal):  no document found in GCLfielddata collection matching query=",
                query,
            )
            exit(-1)
    ...
    if n != 1:
        if n > 1:
            print(
                "project1dmod(WARNING):  duplicate documents for for query=",
                vmquery,
                " of VelocityModel_1d collection",
            )
            print("Will use the first one found")
        else:
            print(
                "project1dmod(Fatal):  no document found in VelocityModel_1d collection matching query=",
                vmquery,
            )
            exit(-1)
