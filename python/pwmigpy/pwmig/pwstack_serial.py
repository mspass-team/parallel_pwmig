...
class pwstack_control:
    ...
    def __init__(
        ...
        if nfound < 1:
            raise MsPASSError(
                "GCLgrid with name=" + gridname + " was not found in database", "Fatal"
            )
        elif nfound > 1:
            raise MsPASSError(
                "GCLgrid with name="
...
def site_query(db, lat, lon, ix1, ix2, cutoff, units="km"):
    ...
    if units == "radians":
        ...
        rcutoff = (
            cutoff / 6371.0
        )  # might want to use a more precise earth radius in km here
    elif units == "degrees":
    ...
    #             ("$maxDistance" , rcutoff) ] )
    #       }
    query = {"coordinates": {"$geoWithin": {"$centerSphere": [[lon, lat], rcutoff]}}}
        ...
        if len(d.member) > 0:
            ...
            if len(srcdata) == 0:
                ...
                georesult = gps2dist_azimuth(
                    srcdata["source_lat"], srcdata["source_lon"], pslat, pslon
                )
...
def pwstack(
    ...
    control = pwstack_control(
        db, pf, slowness_grid_tag, data_mute_tag, stack_mute_tag, save_history, instance
    )
