...
class gridstacker_control:
    ...
    def __init__(self,pf_or_pfname="gridstacker.pf"):
        if isinstance(pf_or_pfname,str):
            ...
            raise ValueError("gridstacker_control constructor:  illegal type for ar0 - must be a pf file name or pf object")
...
def source_cell_weights(db,tcidlist,power=0.5,floor=0.05)->np.ndarray:
    ...
    for tcid in tcidlist:
        ...
        if doc is None:
            ...
            message += "Likely error in use of options for telecluster_id use in pwstack/pwmig"
    ...
    scale = 1.0/maxcount
    weights *= scale # this scaling makes power and floor independent of total data size
...
def stack_data(imagelist,cutoff,weights=None):
    ...
    if weights is not None:
        ...
        if len(imagelist) != len(weights):
            ...
            message += "Does not match length of array of image volumes={}".format(len(imagelist))
            raise RuntimeError(message)
    else:
...
def gridstacker(doclist_or_cursor,
    ...
    for doc in doclist_or_cursor:
        ...
        if count==0:
            ...
            if (fdata.n1 != mastergrid.n1) or fdata.n2 != (mastergrid.n2) or (fdata.n3 != mastergrid.n3):
