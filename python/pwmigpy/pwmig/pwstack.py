...
class pwstack_control:
    ...
    def __init__(
        ...
        if use_fresnel:
            ...
            self.aperture = DepthDependentAperture(
                fvs, fvp, fperiod, fdt, fnt, fcm, True
            )
        else:
...
def read_ensemble(
    ...
    if fold <= control.stack_count_cutoff:
        ...
        if n == 0:
            ...
            d = db.read_data(
                cursor, collection="wf_Seismogram", data_tag=control.data_tag
            )
            cursor.close()
        ...
        if len(d.member) > 0:
            ...
            if len(srcdata) == 0:
                ...
                georesult = gps2dist_azimuth(
                    srcdata["pwmig_source_lat"],
                    srcdata["pwmig_source_lon"],
                    pslat,
                    pslon,
                )
                ...
                if len(arrivals) > 0:
                    ...
                    umag = (
                        ray_param / Rearth
                    )  # need slowness in s/km but ray_param is s/radian
                    baz = georesult[
                        2
                    ]  # The obspy function seems to return back azimuth
                    az = baz + 180.0
                    ...
                    d.kill()
                    message = (
                        "Travel time calculator failed to compute P wave arrival time\n"
                    )
...
def pwstack(
    ...
    control = pwstack_control(
        pf, slowness_grid_tag, data_mute_tag, stack_mute_tag, save_history, instance
    )
    ...
    if nfound < 1:
        ...
        raise MsPASSError(
            "GCLgrid with name="
            + pseudostation_gridname
            + " is ambiguous - multiple documents have that name tag",
            "Fatal",
        )
    ...
    if restart:
        ...
        finished_list = checkpoint_report(
            db, data_tag=output_data_tag, idkey=id2use, print_report=False
        )
        filtered_list = list()
        ...
        if verbose:
            ...
            print(
                f"The following values for {id2use} appear to have been previously processed and will be dropped"
            )
    ...
    if source_collection == "source":
        ...
        srcmatcher = ObjectIdMatcher(
            db,
            collection="telecluster",
            attributes_to_load=["_id", "hypocentroid"],
        )
    else:
        message = "pwstack:   Illegal value for source_collection={}".format(
            source_collection
        )
        message += "Must be either source or telecluster"
        raise ValueError(message)
    sitematcher = ObjectIdMatcher(
        db,
        collection="site",
    ...
    for sid in source_id_list:
        ...
        if minimum_input_data is not None:
            ...
            if n_this_sid < minimum_input_data:
                print("Number of waveforms for sid=", sid, " is ", n_this_sid)
                print(
                    "That number is below minimum_input_data threshold set as ",
                    minimum_input_data,
                )
        ...
        if run_serial:
            ...
            if verbose:
                print(
                    "Starting main processing using parallel algorithm for source with id=",
                    sid,
                )
...
def checkpoint_report(
    ...
    gid_not_defined = 0
    totaldocs = (
        0  # count ourselves for efficiency instead of calling count_documents again
    )
    cursor = db.wf_Seismogram.find(query)
    for doc in cursor:
        if idkey in doc:
    ...
    if print_report:
        ...
        print(
            "If they do not, it means processing of that event was aborted before completion"
        )
        print(
            f"If that is so, edit the wf_Seismogram collection to remove data for that id with data tag={data_tag}"
        )
        print(
            "////////////////////////////////////////////////////////////////////////////////"
        )
        ...
        print(
            f"Number of documents with {idkey} defined but with no data for gridid={gid_not_defined}"
        )
        print(f"Number of documents scanned={totaldocs}")
        print(
            "If any of those are nonzero, some detective work on  wf_Seismogram is recommended"
        )
        print(
            "////////////////////////////////////////////////////////////////////////////////"
        )
