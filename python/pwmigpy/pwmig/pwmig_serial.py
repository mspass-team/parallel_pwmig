...
from pwmigpy.ccore.gclgrid import GCLvectorfield3d, PWMIGfielddata
...
from pwmigpy.db.database import GCLdbread, vmod1d_dbread_by_name, GCLdbread_by_name
...
def BuildSlownessGrid(
    g, source_lat, source_lon, source_depth, model="iasp91", phase="P"
):
...
def _migrate_component(cursor, db, parent, TPfield, VPsvm, Us3d, Vp1d, Vs1d, control):
    ...
    for doc in cursor:
        ...
        migseis = migrate_one_seismogram(
...
def pwmig_verify(
    db, pffile="pwmig.pf", GCLcollection="GCLfielddata", check_waveform_data=False
):
    """
    ...
    """
    print(
        "Warning:  this function is not yet implemented.   Checks only if pf is readable"
    )
...
def migrate_event(db, source_id, pf, collection="GCLfielddata"):
    ...
    if doc == None:
        ...
        raise MsPASSError(
            "migrate_event:  source_id=" + str(source_id) + " not found in database",
            ErrorSeverity.Fatal,
        )
    source_lat = doc["lat"]
    source_lon = doc["lon"]
    ...
    TPfield = ComputeIncidentWaveRaygrid(
        parent, border_pad, Up3d, Vp1d, svm0, zmax * zpad, tmax, dt, zdecfac, True
    )
    del Up3d
    ...
    for gridid in gridid_list:
        ...
        migrated_data = _migrate_component(
            cursor, db, parent, TPfield, svm0, Us3d, Vp1d, Vs1d, control
        )
