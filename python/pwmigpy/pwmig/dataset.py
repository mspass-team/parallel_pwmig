...
def pwmig_dataset(
    ...
    if initialize_workers:
        ...
        print(
            "This job may abort if workers were not previously initalized with the MongoDBWorker plugin"
        )
    ...
    if restart:
        ...
        print(
            "Found ",
            len(completed),
            " documents in GCLfielddata that appear to be pwmig output",
        )
        newlist = list()
        for tcid in idlist:
...
def pwstack_dataset(
    ...
    if wf_query is None:
        ...
        if pseudosource_stacker_algorithm in valid_algorithms:
            ...
            message = "pwstack_dataset:  Illegal value for pseudosource_stacker_algorith={}".format(
                pseudosource_stacker_algorithm
            )
